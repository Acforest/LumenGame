...
import { _getRepositoryGame, _searchRepositoryGame, _searchRepositoryTag } from '@api'
...
export default {
  ...
  methods: {
    ...
    // 搜索游戏
    async searchGame() {
      const { status, message, data } = await _searchRepositoryGame({'keyword': this.keyword, 'user_id': this.currentUser.id})
      this.gameList = JSON.parse(data)
      this.total = this.gameList ? this.gameList.length : 0
      this.handleCategory()
    },
    // 搜索标签
    async searchTag(tag) {
      const { status, message, data } = await _searchRepositoryTag({'tag': tag, 'user_id': this.currentUser.id})
      this.gameList = JSON.parse(data)
      this.total = this.gameList ? this.gameList.length : 0
      this.handleCategory()
    },
